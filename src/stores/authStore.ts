...
// Auth error class with i18n key support
...
function processMockAuth(credential: string): AuthUser {
  ...
  const expectedNonce = getCurrentNonce()
  if (!expectedNonce || payload.nonce !== expectedNonce) {
    authStoreLogger.warn('Nonce mismatch - possible CSRF attack', {
      received: typeof payload.nonce === 'string' ? payload.nonce.slice(0, 8) + '...' : 'none',
...
// Auth request state managed atomically
interface AuthRequestState {
  pendingRequests: Map<string, PendingAuthRequest>
...
function cleanupPendingRequests() {
  ...
  for (const [id, request] of authRequestState.pendingRequests.entries()) {
    if (now - request.timestamp > maxAge) {
      request.reject(new Error('Auth request timeout'))
      authRequestState.pendingRequests.delete(id)
    }
  }
}
  ...
  persist(
    (set, get) => ({
      ...
      signInWithGoogle: async () => {
        ...
        // Generate unique request ID atomically
        const requestId = crypto.randomUUID()
        authRequestState.currentRequestId = requestId

        return new Promise<void>((resolve, reject) => {
          authRequestState.pendingRequests.set(requestId, {
            resolve,
            reject,
            timestamp: Date.now(),
          })
          ...
          promptGoogleSignIn().catch((error) => {
            ...
            const pending = authRequestState.pendingRequests.get(requestId)
            if (pending && authRequestState.currentRequestId === requestId) {
              pending.reject(error)
              authRequestState.pendingRequests.delete(requestId)
              authRequestState.currentRequestId = null
            }
          })
        })
      },
      ...
      _handleGoogleCallback: async (response: GoogleCredentialResponse) => {
        const requestId = authRequestState.currentRequestId
        const pending = requestId ? authRequestState.pendingRequests.get(requestId) : null
        ...
        try {
          ...
          if (requestId) {
            authRequestState.pendingRequests.delete(requestId)
            if (authRequestState.currentRequestId === requestId) {
              authRequestState.currentRequestId = null
            }
          }
        }
