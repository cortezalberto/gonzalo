...
export const useCartItems = () => useTableStore((state) => state.session?.shared_cart || [])
export const useDiners = () => useTableStore((state) => state.session?.diners || [])
...
export const useHeaderData = () => {
  ...
  const diners = useMemo(() => session?.diners || [], [session?.diners])
...
export const useSharedCartData = () => {
  ...
  // Compute derived values with useMemo
  const cartItems = useMemo(() => session?.shared_cart || [], [session?.shared_cart])
  const diners = useMemo(() => session?.diners || [], [session?.diners])
