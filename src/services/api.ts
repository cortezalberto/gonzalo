...
// Allowed hosts list to prevent SSRF
...
// Components can use these to show appropriate UI and retry logic
export const API_ERROR_CODES = {
  TIMEOUT_ERROR: 'TIMEOUT_ERROR',       // Request took too long
  NETWORK_ERROR: 'NETWORK_ERROR',       // No internet or server unreachable
  EMPTY_RESPONSE: 'EMPTY_RESPONSE',     // Server returned empty body
  PARSE_ERROR: 'PARSE_ERROR',           // Invalid JSON response
  UNKNOWN_ERROR: 'UNKNOWN_ERROR',       // Unexpected error
  HTTP_ERROR: 'HTTP_ERROR',             // Server returned error status
} as const
...
export class ApiError extends Error {
  ...
  // Helper to determine if error is recoverable with retry
  get isRetryable(): boolean {
    return this.code === API_ERROR_CODES.TIMEOUT_ERROR ||
           this.code === API_ERROR_CODES.NETWORK_ERROR ||
           (this.status >= 500 && this.status < 600)
  }

  // Helper to determine if it's a client error (bad request)
  get isClientError(): boolean {
    return this.status >= 400 && this.status < 500
  }

  // Returns i18n key for user-friendly error message
  // Components should use t(error.i18nKey) to display translated message
  get i18nKey(): string {
    switch (this.code) {
      ...
      case API_ERROR_CODES.EMPTY_RESPONSE:
        return 'errors.emptyResponse'
      case API_ERROR_CODES.PARSE_ERROR:
...
async function request<T>(endpoint: string, options: RequestOptions = {}): Promise<T> {
  ...
  const { timeout = 30000, skipDedup = false, ...fetchOptions } = options
